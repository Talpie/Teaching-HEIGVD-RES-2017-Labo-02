	    ...
	    while (!done && ((command = reader.readLine()) != null)) {
	      ...
	      switch (command.toUpperCase()) {
	        ...
	        case RouletteV2Protocol.CMD_LOAD:
	          ...
	          writer.flush();
	          int initialStudents = store.getNumberOfStudents();
	          LoadCommandResponse loadResponse;
	          try {
	        	  store.importData(reader);
	        	  loadResponse = new LoadCommandResponse(Status.sucess, store.getNumberOfStudents() - initialStudents);
			} catch (Exception e) {
				 loadResponse = new LoadCommandResponse(Status.failure, 0);
			}
	        ...
	        case RouletteV2Protocol.CMD_BYE:
	          ...
	          ByeCommandResponse byeResponse = new ByeCommandResponse(Status.sucess,nbCommands);
